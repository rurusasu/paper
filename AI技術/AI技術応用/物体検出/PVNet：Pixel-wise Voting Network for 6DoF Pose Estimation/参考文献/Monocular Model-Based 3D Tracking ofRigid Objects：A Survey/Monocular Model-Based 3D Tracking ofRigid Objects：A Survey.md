...
### 2.1.1. The Perspective Project Model
...
画像形成は、数学的には、図 2.1 に示すように、3 次元空間から画像平面への投影として定義することができる。ユークリッド世界座標系で押し出された 3 次元点 $M=[X, Y, Z]$の座標と画像上の対応する 2 次元点 $\bf{m}=[u, v]^T$ は次式で表される。
...
$$
\bf{P} = K \left[\bf{R}|\bf{t} \right]
$$

ここで、

* $Ｋ$ は、焦点距離などのカメラの内部パラメータに依存する $３×３$ カメラキャリブレーションマトリクスである。
* $\left[\bf{R}|\bf{t} \right]$ は $3 × 4$ の外部パラメータ行列であり、ワールド座標系からカメラ座標系へのユークリッド変換に相関します。以下、両者について詳しく説明する。

### 2.1.2. The Cammera Calibration Matrix

カメラキャリブレーション行列 $K$ には，内部パラメータとも呼ばれるカメラ固有のパラメータが含まれています．ここでは，次のように書きます．
...
* $\bf{c}=[u_0,v_0]^T$ は、光軸と像面の交点の画像座標を表し、主点とも呼ばれる。
